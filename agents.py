...
class MultiTask:
    def __init__(self, state, state_size, action_size, num_outputs_1=5, num_outputs_2=5, task='dqn'):
        self.state = state
    ...
    def replay_dqn(self, batch_size):
        ...
        target_Qs[np.arange(batch_size), actions] = rewards + self.dqn_gamma * np.max(target_Qs_next, axis=1) * (1 - dones)
    ...
    def replay_actor_critic(self, batch_size):
        ...
        actions = np.array([e[1] for e in experiences])[:, np.newaxis]
        rewards = np.array([e[2] for e in experiences])[:, np.newaxis]
        next_states = np.array([e[3] for e in experiences])
        dones = np.array([e[4] for e in experiences])[:, np.newaxis]
        # Predict the value of the next state
        states = np.reshape(states, (-1, self.state_size))
        next_states = np.reshape(next_states, (-1, self.state_size))
        next_values = self.actor_critic_model.predict(next_states)
        # Calculate the target value
        targets = rewards + self.actor_critic_gamma * (1 - dones) * next_values
        # Fit the actor-critic model
        self.actor_critic_model.fit(states, [targets, actions], epochs=1, verbose=0)
    ...
    def replay_policy_gradient(self, batch_size):
        ...
        target_q_values = self.policy_gradient_model.predict(states)
        target_q_values[np.arange(batch_size), actions] = rewards + self.policy_gradient_gamma * np.max(self.policy_gradient_model.predict(next_states), axis=1) * (1 - dones)
    
        # Fit the model on the experiences
        self.policy_gradient_model.fit(states, target_q_values, epochs=1, verbose=0)
        # print("333333333")
    ...
    def act(self, state, task):
        ...
        if task == 'dqn':
            ...
            if np.random.rand() <= self.ddqn_epsilon:
                return random.randrange(self.action_size)
            q_values = self.ddqn_model.predict(state)
            return np.argmax(q_values[0])
        elif task == 'actor_critic':
            probs = self.actor_critic_model.predict(state)
            return np.random.choice(self.action_size, p=probs[0].reshape(-1))
        elif task == 'policy_gradient':
            probs = self.policy_gradient_model.predict(state)[0]
            return np.random.choice(self.action_size, p=probs.reshape(-1))
        
    def load(self,name,task):
        if task == 'dqn':
            self.dqn_model.load_weights(f'{name}_dqn.h5')
        elif task == 'ddqn':
            self.ddqn_model.load_weights(f'{name}_ddqn.h5')
        elif task == 'actor_critic':
            self.actor_critic_model.load_weights(f'{name}_actor_critic.h5')
        elif task == 'policy_gradient':
            self.policy_gradient_model.load_weights(f'{name}_policy_gradient.h5')
